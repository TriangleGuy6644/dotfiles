...
func main() {
	...
	case "fileserver":
		...
		fmt.Printf("\ntry: ")
		color.Green("sudo ufw allow 8000 && sudo ufw reload")
		runCmd("python -m http.server")
	case "webdl":
		if len(args) < 3 {
			fmt.Println(color.HiBlackString("webdl requires atleast one argument. try:") + color.GreenString(" webdl <url>"))
			os.Exit(0)
		}
		url := args[2]
		runCmd("wget --random-wait -r -p -e robots=off -U mozilla " + url)
	case "geniso":
		//second argument is the disk, third is the path to image
		if len(args) < 4{
			fmt.Println(color.HiBlackString("geniso requires arguments. try: ") + color.GreenString("geniso </dev/[disk]> /path/to/image.iso"))
			os.Exit(0)
		}
		disk := args[2]
		imgPath := args[3]
		intRunCmd(fmt.Sprintf("readom "))
...
fileserver: host the current directory as a web server, similar to a file hoster.
webdl: downlaod an entire website. hint: serve it by cd'ing into the directory and running fileserver
geniso: generate an iso from a disk`
	switch c {
	case "red":
		color.Red(helpmsg)
	case "blue":
